        ...
        where TInner : class
    {
        ...
        private class WeakRefEqualityComparator : IEqualityComparer<WeakReference<TInner>>
        {
            public bool Equals(WeakReference<TInner> x, WeakReference<TInner> y)
            {
                ...
                {
                    TInner xInner;
                    bool gotX = x.TryGetTarget(out xInner);

                    TInner yInner;
                    bool gotY = y.TryGetTarget(out yInner);
        ...
        {
            WeakReference<TInner> innerWeakRef = new WeakReference<TInner>(inner);
            TOuter result;
            ...
            Func<TInner, TOuter> projectDelegate)
        {
            ...
            for (int idx = 0; idx < outerCollection.Count(); ++idx)
            {
        ...
        private void InnerCollectionChanged(object sender, NotifyCollectionChangedEventArgs innerEventArgs)
        {
            switch (innerEventArgs.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    {
                        ...
                        for (int idx = 0; idx < outerNewItems.Count(); ++idx)
                        {
                ...
                case NotifyCollectionChangedAction.Remove:
                    {
                        int count = innerEventArgs.OldItems.Count;
                ...
                case NotifyCollectionChangedAction.Move:
                    {
                        int count = innerEventArgs.OldItems.Count;
                        int oldIdx = innerEventArgs.OldStartingIndex;
                        int newIdx = innerEventArgs.NewStartingIndex;
                ...
                case NotifyCollectionChangedAction.Reset:
                    {
                        ...
                        int commonCount = Math.Min(outerNewItems.Count, this.Count);
                        for (int idx = 0; idx < commonCount; ++idx)
                        {
                            SetItem(idx, outerNewItems[idx]);
                        }
                        // If there are extra items in the current collection trim the end
                        while (this.Count > outerNewItems.Count)
                        {
                            RemoveAt(this.Count - 1);
                        }
                        // If there isn't enough space in the current collection append to the end
                        for (int idx = this.Count; idx < outerNewItems.Count; ++idx)
                        {
