...
using static WebView2Utilities.Core.Models.AppOverrideEntry;
...
public class AppOverrideList : ObservableCollection<AppOverrideEntry>
{
    ...
    {
        if (!appNameToEntry.TryGetValue(valueName, out var entry))
        {
    ...
    public static void UpdateCollectionFromRegistry(ObservableCollection<AppOverrideEntry> collection, StorageKind storageKind)
    {
        ...
        var appNameToEntry = new Dictionary<string, AppOverrideEntry>();
        var entriesToRemove = new HashSet<AppOverrideEntry>();
        RegistryKey regKey;
        string[] valueNames;
        var registryRoot = storageKind == StorageKind.HKLM ? Registry.LocalMachine : Registry.CurrentUser;
        ...
        {
            var entry = GetOrCreateEntry(appNameToEntry, collection, valueName, storageKind);
        ...
        foreach (var valueName in valueNames)
        {
            ...
            try
            {
                ...
                {
                    if (int.TryParse((string)value, out var valueAsInt))
                    {
        ...
        {
            var entry = new AppOverrideEntry
            {
                HostApp = "*",
    ...
    private static Channels ReleaseChannelsFromString(string channelString)
    {
        ...
        if (!string.IsNullOrEmpty(channelString))
        {
            ...
            {
                if (int.TryParse(channelNumber.Trim(), out var number))
                {
                    switch (number)
                    {
                        case 0:
    ...
    {
        var collection = new ObservableCollection<AppOverrideEntry>();
    ...
    {
        var target = storageKind == StorageKind.EVLM ?
            EnvironmentVariableTarget.Machine : EnvironmentVariableTarget.User;
        var envVars = Environment.GetEnvironmentVariables(target);
        var browserExecutableFolder = (string)envVars["WEBVIEW2_BROWSER_EXECUTABLE_FOLDER"];
        var userDataFolder = (string)envVars["WEBVIEW2_USER_DATA_FOLDER"];
        var additionalBrowserArguments = (string)envVars["WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS"];
        var releaseChannelPreferenceAsString = (string)envVars["WEBVIEW2_RELEASE_CHANNEL_PREFERENCE"];
        var channelSearchKindAsString = (string)envVars["WEBVIEW2_CHANNEL_SEARCH_KIND"];
        var releaseChannelsAsString = (string)envVars["WEBVIEW2_RELEASE_CHANNELS"];
        var reverseChannelSearch = releaseChannelPreferenceAsString == "1" || channelSearchKindAsString == "1";

        var foundInCollection = true;
        var entry = collection.FirstOrDefault(existingEntry => existingEntry.StorageKind == storageKind);
        if (entry == null)
        {
            entry = new AppOverrideEntry();
    ...
    public static void ApplyToRegistry(IEnumerable<AppOverrideEntry> newEntries, StorageKind storageKind)
    {
        if (!IgnoreUpdatesToRegistry)
        {
            ...
            var filteredNewEntries = newEntries.Where(e => e.StorageKind == storageKind);
            foreach (var entry in registryEntries.Except(filteredNewEntries, new AppOverrideEntryHostAppEquality()))
            {
    ...
    {
        var target = entry.StorageKind == StorageKind.EVLM ?
            EnvironmentVariableTarget.Machine : EnvironmentVariableTarget.User;
    ...
    private static void ApplyEntryToRegistry(AppOverrideEntry entry)
    {
        if (!IgnoreUpdatesToRegistry)
        {
            ...
            else
            {
                ...
                var releaseChannelsAsString = ReleaseChannelsToString(entry.ReleaseChannels);
...
public class AppOverrideEntry : INotifyPropertyChanged
{
    public string PrecedenceCategory
    {
        get
        {
            ...
            var precedence = (int)StorageKind * 2;
            if (HostApp == "*")
            {
                ++precedence;
            }
    ...
    protected void OnPropertyChanged(string name)
    {
        ...
        if (m_InitializationComplete)
        {
            ...
            {
                var removeRoot = StorageKind == StorageKind.HKCU ? Registry.LocalMachine : Registry.CurrentUser;
    ...
    private string NullToEmpty(string inp)
    {
        return inp == null ? "" : inp;
    }
}
